...
rPlayer.prototype.handleSocketEvent = function(evt, select) {
    var select = (select === undefined) ? true : select;

    if (evt instanceof Array) {
        for (var i in evt) {
            this.handleSocketEvent(evt[i], select);
        }
        return;
    }
    ...
    // LMB was pressed. Hence we are either (a) selecting a single unit,
    // (b) deselecting units or (c) doing nothing.
    if (select && evt.type == "mousedown" && evt.button == 0) {
    ...
    // (b) finishing a selection, or (c) doing nothing.
        ...
        // Stop selecting.
        if (select && evt.button == 0 && this.selectionBox !== null) {
            this.adjustSelection(position);
            this.stopSelecting();
...
rPlayer.prototype.adjustSelection = function(pos) {
    this.selectionBox.w = pos.x;
    this.selectionBox.h = pos.y;
